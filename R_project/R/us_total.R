clean_2005_us <- function(df, year, month) {
  ...
  df_filtered <- df %>%
    rename(
      ...
      pais = "...2",) %>%
    mutate(year = year) %>%
    filter(pais == "Total ESTADOS UNIDOS DE AMERICA")  %>%
    select_if(~!all(is.na(.)))
  ...
  final_df <- reset_us_info(df_filtered)
  
}

clean_2006_us <- function(df, year, month) {
...
clean_2012_us <- function(df, year, month) {
  ...
  df_filtered <- df %>%
    rename(
      ...
      pais = "...2") %>%
    filter(pais_id %in% c(
      "Total ESTADOS UNIDOS DE AMERICA",
      "Total ESTADOS UNIDOS DE AMÉRICA"
      )
    ) %>%
    mutate(year = year)  %>%
    select_if( ~ !all(is.na(.)))
...
clean_2014_us <- function(df, year, month) {
  df_filtered <- df %>%
    rename(
      consulado_id = "...3",
      consulado = "...4",
...
clean_2021_us <- function(df, year) {
