...
const projection = {
    ...
    geoAlbersUsaPr: function() {
        ...
        albersUsa.stream = function(stream) {
          return cache && cacheStream === stream ? cache : cache = projection.multiplex([lower48.stream(cacheStream = stream), alaska.stream(stream), hawaii.stream(stream), puertoRico.stream(stream)]);
        };
        ...
        albersUsa.translate = function(_) {
          ...
          alaskaPoint = alaska
              ...
              .clipExtent([[x - 0.425 * k + projection.epsilon, y + 0.120 * k + projection.epsilon], [x - 0.214 * k - projection.epsilon, y + 0.234 * k - projection.epsilon]])
              .stream(pointStream);
      
          hawaiiPoint = hawaii
              .translate([x - 0.205 * k, y + 0.212 * k])
              .clipExtent([[x - 0.214 * k + projection.epsilon, y + 0.166 * k + projection.epsilon], [x - 0.115 * k - projection.epsilon, y + 0.234 * k - projection.epsilon]])
              .stream(pointStream);
